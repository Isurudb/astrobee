...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      buff.write(_struct_I.pack(length))
      pattern = '<%sf'%length
      buff.write(struct.pack(pattern, *self.avg_loads))
      buff.write(_get_struct_f().pack(self.temp))
      length = len(self.cpus)
      buff.write(_struct_I.pack(length))
      for val1 in self.cpus:
        buff.write(_get_struct_B().pack(val1.enabled))
        length = len(val1.loads)
        buff.write(_struct_I.pack(length))
        pattern = '<%sf'%length
        buff.write(struct.pack(pattern, *val1.loads))
        _x = val1
      ...
      for val1 in self.load_nodes:
        ...
        if python3 or type(_x) == unicode:
          ...
          length = len(_x)
        buff.write(struct.pack('<I%ss'%length, length, _x))
        buff.write(_get_struct_f().pack(val1.load))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.avg_loads = struct.unpack(pattern, str[start:end])
      start = end
      end += 4
      ...
      for i in range(0, length):
        ...
        val1.loads = struct.unpack(pattern, str[start:end])
        _x = val1
        start = end
        end += 8
